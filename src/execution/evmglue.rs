use super::{
    ...
    continuation::{
        interrupt_data::InterruptData as StateInterruptData, resume_data::ResumeData,
        StateGenerator,
    },
    evm::AnalyzedCode,
    precompiled,
...
pub fn execute<'s>(
    state: &'s mut IntraBlockState,
    tracer: Option<&'s mut dyn Tracer>,
    analysis_cache: &'s mut AnalysisCache,
    header: &'s PartialHeader,
    block_spec: &'s BlockExecutionSpec,
    txn: &'s MessageWithSender,
    gas: u64,
) -> impl Generator<ResumeData, Yield = StateInterruptData, Return = CallResult> + 's {
    move |_| {
        let mut evm = Evm {
            header,
            tracer,
        ...
        let res = if let TransactionAction::Call(to) = txn.action() {
            ...
            gen_await!(evm.create(CreateMessage {
                depth: 0,
                gas: gas as i64,
                sender: txn.sender,
...
impl<'state, 'tracer, 'analysis, 'h, 'c, 't> Evm<'state, 'tracer, 'analysis, 'h, 'c, 't> {
    fn create(&mut self, message: CreateMessage) -> StateGenerator<'_, Output> {
        Box::pin(move |_| {
            let mut res = Output {
...
mod tests {
    ...
    fn create_should_only_return_on_failure() {
        InMemoryState::new().execute(StartedInterrupt::from(Box::pin(|_| {
            let header = PartialHeader {
    ...
    fn contract_overwrite() {
        InMemoryState::new().execute(StartedInterrupt::from(Box::pin(|_| {
            ...
            let mut state = IntraBlockState::new();
            gen_await!(state.set_code(contract_address, old_code.to_vec().into()));
    ...
    fn eip3541() {
        InMemoryState::new().execute(StartedInterrupt::from(Box::pin(|_| {
            ...
            assert_eq!(
                gen_await!(execute(&mut state, &header, &txn, gas)).status_code,
